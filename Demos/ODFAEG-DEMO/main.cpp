...
public :
    TestAppli(sf::VideoMode wm, std::string title) : Application (wm, title, sf::Style::Default, ContextSettings(0, 0, 4, 4, 6)) {
    ...
    /*void gaignedFocus(gui::TextArea* textArea) {
        std::cout<<"gaigned focus"<<std::endl;
    }*/
    void keyHeldDown (IKeyboard::Key key) {
    ...
    void onInit () {
        ...
        emitter.setParticleColor(Distributions::color(Vec3f(255, 255, 255, 255), Vec3f(255, 255, 255, 255)));
       /* sf::Vector3f acceleration(0, -1, 0);
        ...
        caracter->setCenter(Vec3f(-25, -50, 0));
        ...
        Command moveAction(combined, FastDelegate<void>(&TestAppli::keyHeldDown, this, IKeyboard::Unknown));
        getListener().connect("MoveAction", moveAction);
        if (!day)
            g2d::AmbientLight::getAmbientLight().setColor(sf::Color(0, 0, 255));
        Command mouseInsideAction(a5, FastDelegate<bool>(&TestAppli::mouseInside,this, sf::Vector2f(-1, -1)), FastDelegate<void>(&TestAppli::onMouseInside, this, sf::Vector2f(-1,-1)));
        getListener().connect("MouseInside",mouseInsideAction);
        Command leftMouseButtonPressedAction (a5, FastDelegate<void>(&TestAppli::leftMouseButtonPressed, this, sf::Vector2f(-1, -1)));
    ...
    void onUpdate (RenderWindow* rw, IEvent& event) {
