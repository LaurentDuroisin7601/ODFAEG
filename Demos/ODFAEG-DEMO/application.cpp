...
namespace sorrok {
    ...
    void MyAppli::onMouseInside (math::Vec2f mousePos) {
    ...
    void MyAppli::onLoad() {
        ...
        tm.fromFileWithAlias("tilesets/murs.png", "WALLS", rArgs);
        tm.fromFileWithAlias("tilesets/maison.png", "HOUSE", rArgs);
        tm.fromFileWithAlias("tilesets/flemmes1.png", "FIRE1", rArgs);
        tm.fromFileWithAlias("tilesets/flemmes2.png", "FIRE2", rArgs);
        tm.fromFileWithAlias("tilesets/flemmes3.png", "FIRE3", rArgs);
        tm.fromFileWithAlias("tilesets/particule.png", "PARTICLE", rArgs);
        std::string path = "tilesets/vlad_sword.png";
        tm.fromFileWithAlias(path, "VLADSWORD", rArgs);
    ...
    void MyAppli::onInit () {
        ...
        tiles.push_back(entityFactory.make_entity<Tile>(tm.getResourceByAlias("WATER"), Vec3f(0, 0, 0), Vec3f(120, 60, 0),IntRect(0, 0, 100, 50), entityFactory));
        walls.resize(g2d::Wall::NB_WALL_TYPES, nullptr);
        //tiles.push_back(new Tile(tm.getResourceByAlias("GRASS"), Vec3f(0, 0, 0), Vec3f(120, 60, 0),IntRect(0, 0, 100, 50)));
        walls[g2d::Wall::TOP_BOTTOM] = entityFactory.make_entity<g2d::Wall>(entityFactory.make_entity<Tile>(tm.getResourceByAlias("WALLS"), Vec3f(0, 0, 0), Vec3f(100, 100, 0), IntRect(100, 0, 100, 100), entityFactory),g2d::Wall::TOP_BOTTOM,&g2d::AmbientLight::getAmbientLight(), entityFactory);
        //std::cout<<"global bounds : "<<walls[g2d::Wall::TOP_BOTTOM]->getGlobalBounds().getPosition()<<std::endl;
        walls[g2d::Wall::RIGHT_LEFT] = entityFactory.make_entity<g2d::Wall>(entityFactory.make_entity<Tile>(tm.getResourceByAlias("WALLS"), Vec3f(0, 0, 0), Vec3f(100, 100, 0), IntRect(100, 100, 100, 100), entityFactory),g2d::Wall::RIGHT_LEFT,&g2d::AmbientLight::getAmbientLight(), entityFactory);
        walls[g2d::Wall::BOTTOM_RIGHT] = entityFactory.make_entity<g2d::Wall>(entityFactory.make_entity<Tile>(tm.getResourceByAlias("WALLS"), Vec3f(0, 0, 0), Vec3f(100, 100, 0), IntRect(100, 200, 100, 100), entityFactory),g2d::Wall::BOTTOM_RIGHT,&g2d::AmbientLight::getAmbientLight(), entityFactory);
        walls[g2d::Wall::TOP_LEFT] = entityFactory.make_entity<g2d::Wall>(entityFactory.make_entity<Tile>(tm.getResourceByAlias("WALLS"), Vec3f(0, 0, 0), Vec3f(100, 100, 0), IntRect(100, 300, 100, 100), entityFactory),g2d::Wall::TOP_LEFT,&g2d::AmbientLight::getAmbientLight(), entityFactory);
        walls[g2d::Wall::TOP_RIGHT] = entityFactory.make_entity<g2d::Wall>(entityFactory.make_entity<Tile>(tm.getResourceByAlias("WALLS"), Vec3f(0, 0, 0), Vec3f(100, 100, 0), IntRect(100, 400, 100, 100), entityFactory),g2d::Wall::TOP_RIGHT,&g2d::AmbientLight::getAmbientLight(), entityFactory);
        walls[g2d::Wall::BOTTOM_LEFT] = entityFactory.make_entity<g2d::Wall> (entityFactory.make_entity<Tile>(tm.getResourceByAlias("WALLS"), Vec3f(0, 0, 0), Vec3f(100, 100, 0), IntRect(100, 500, 100, 100), entityFactory), g2d::Wall::BOTTOM_LEFT,&g2d::AmbientLight::getAmbientLight(), entityFactory);
        ...
        if(ifs) {
            ...
            for (unsigned int i = 0; i < entities.size(); i++) {
                ...
                if (entities[i]->getType() == "E_BIGTILE") {
                    for (unsigned int j = 0; j < entities[i]->getChildren().size(); j++) {
                        ...
                        IntRect texRect = entities[i]->getChildren()[j]->getFace(0)->getMaterial().getTexRect();
                    ...
                    IntRect texRect = entities[i]->getChildren()[0]->getFace(0)->getMaterial().getTexRect();
                    ...
                    for (unsigned int j = 0; j < anim->getChildren().size(); j++) {
                        ...
                        IntRect texRect = entities[i]->getChildren()[j]->getChildren()[0]->getFace(0)->getMaterial().getTexRect();
            ...
            decor->setShadowCenter(Vec3f(0, 400, -200));
            ...
            fire1->updateTransform();
            fire2->updateTransform();
            fire3->updateTransform();
            fire->addFrame(fire1);
            fire->addFrame(fire2);
            fire->addFrame(fire3);
            fire->play(true);
        ...
        //std::cout<<"ps texture id : "<<tm.getResourceByAlias("PARTICLE")->getId()<<std::endl;
        ...
        emitter.setParticleColor(Distributions::color(Vec4f(255, 255, 255, 255), Vec4f(255, 255, 255, 255)));
       /* sf::Vector3f acceleration(0, -1, 0);
        ...
        ShadowRenderComponent *src = new ShadowRenderComponent(getRenderWindow(), 3, "E_BIGTILE+E_WALL+E_DECOR+E_ANIMATION+E_HERO", ContextSettings(0, 0, 4, 4, 6));
        ...
        if (!day)
            g2d::AmbientLight::getAmbientLight().setColor(Color(0, 0, 255));
        Command mouseInsideAction(a5, FastDelegate<bool>(&MyAppli::mouseInside,this, math::Vec2f(-1, -1)), FastDelegate<void>(&MyAppli::onMouseInside, this, math::Vec2f(-1,-1)));
        getListener().connect("MouseInside",mouseInsideAction);
        Command leftMouseButtonPressedAction (a5, FastDelegate<void>(&MyAppli::leftMouseButtonPressed, this, math::Vec2f(-1, -1)));
    ...
    void MyAppli::onRender(RenderComponentManager *cm) {
        ...
        getWorld()->drawOnComponents("E_BIGTILE+E_WALL+E_DECOR+E_ANIMATION+E_HERO", 3);
    ...
    void MyAppli::onUpdate (RenderWindow* rw, IEvent& event) {
        ...
        if (event.type == IEvent::MOUSE_MOTION_EVENT) {
            math::Vec2f mousePos = math::Vec2f(event.mouseMotion.x, event.mouseMotion.y);
