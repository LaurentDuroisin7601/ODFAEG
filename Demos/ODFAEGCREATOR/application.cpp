...
void ODFAEGCreator::findLastBracket(std::string& fileContent, int nbBlocks, int& p) {
    ...
    if (dp == dpGoToMFunc) {
        ...
        for (unsigned int f = 0; f < mf.size(); f++) {
            ...
            if (signature == signatureToFind) {
                ...
                Vec3f cursorPos = Vec3f(tScriptEdit->getCursorPositionLocal().x() - 200, tScriptEdit->getCursorPositionLocal().y(), 0);
