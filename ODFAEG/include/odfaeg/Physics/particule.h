...
namespace odfaeg
{
    namespace physic {
        ...
        explicit	Particle(core::Time totalLifetime);
        ...
        alignas(16) math::Vec3f	position;	///< Current position.
        alignas(16) math::Vec3f	velocity;	///< Velocity (change in position per second).
        alignas(16) math::Vec3f	scale;	///< Scale, where (1,1) represents the original size.
        graphic::Color	color;	///< %Particle color.
        ...
        // Private variables
        private:
        core::Time	passedLifetime;	// Time passed since emitted.
        core::Time	totalLifetime;	// Total time to live.
        int padding;
        ...
        friend core::Time ODFAEG_PHYSICS_API getElapsedLifetime(const Particle& particle);
        friend core::Time ODFAEG_PHYSICS_API getTotalLifetime(const Particle& particle);
        /// @endcond
        };

        /// @relates Particle
        /// @brief Returns the time passed since the particle has been emitted.
        core::Time ODFAEG_PHYSICS_API	getElapsedLifetime(const Particle& particle);

        /// @relates Particle
        /// @brief Returns the total time the particle is alive.
        core::Time ODFAEG_PHYSICS_API	getTotalLifetime(const Particle& particle);

        /// @relates Particle
        /// @brief Returns the time left until the particle dies.
        core::Time ODFAEG_PHYSICS_API	getRemainingLifetime(const Particle& particle);
