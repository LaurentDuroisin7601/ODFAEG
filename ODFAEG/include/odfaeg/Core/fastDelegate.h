...
namespace odfaeg {
    namespace core {
        ...
        */
        ...
        struct IRefVal {
            ...
            *  \return the copied wrapper.*/
            virtual std::unique_ptr<IRefVal<T, LateParamsT, isCopiable>> clone() const = 0;
            /**\fn destructor*/
            virtual ~IRefVal(){}
        ...
        *  \date 1/02/2014
        */
        template <typename T, typename LateParamsT, bool isCopiable>
        class RefVal;
        template<class T, typename LateParamsT, bool isCopiable>
        struct Ref : IRefVal<T, LateParamsT, isCopiable> {
            /**
            ...
            */
            std::unique_ptr<IRefVal<T, LateParamsT, isCopiable>> clone() const
        ...
        */
        ...
        struct Val<T, LateParamsT, true> : IRefVal<T, LateParamsT, true> {
            ...
            * \return std::unique_ptr<IRefVal<T>> : the cloned wrapper.
            */
            std::unique_ptr<IRefVal<T, LateParamsT, true>> clone() const
        ...
        struct Val<T, LateParamsT, false> : IRefVal<T, LateParamsT, false> {
            ...
            * \return std::unique_ptr<IRefVal<T>> : the cloned wrapper.
            */
            std::unique_ptr<IRefVal<T, LateParamsT, false>> clone() const
            { return std::make_unique<Val>(*this); }
        private:
            T val; /**> T val : keep the value of the wrapper.*/
        };
        ...
        */
        ...
        struct RefVal {
            ...
            */
            RefVal(const T& t)
            : rv(std::make_unique<Val<T, LateParamsT, isCopiable>>(t))
            {}
            /**
            ...
            */
            ...
            : rv(std::make_unique<Ref<T, LateParamsT, isCopiable>>(r))
            {}
            /**
            * \fn RefVal (ph<I,T>&&)
            ...
            */
            ...
            RefVal(ph<I,T>&&) //we need to use a different placeholder class here to pass the palceholders's holder type for the static_cast.
                : rv(std::make_unique<placeholder<I,T,LateParamsT, isCopiable>>())
            {}
            ...
            */
            ...
            { return rv->get(); }
        private:
            std::unique_ptr<IRefVal<T, LateParamsT, isCopiable>> rv; /**> a pointer to the generic wrapper interface.*/
        };
      ...
      = jln::mp::copy_if<jln::mp::lift<is_placeholder>,
        ...
        ::f<std::remove_cv_t<ArgT>...>;*/
            std::tuple<RefVal<ToStore_t<ArgT>, late_params_t, std::is_copy_constructible_v<ToStore_t<ArgT>>>...> param; /**> the wrapped values of the parameters to pass to the callback's function.*/
            std::tuple<RefVal<ToStore_t<ArgT>, late_params_t, std::is_copy_constructible_v<ToStore_t<ArgT>>>...> tmpParam; /**> the wrapped values of the temporary parameters (parameters with placeholder's types).*/
        };
        /**
        *  \file  fastDelegate.h
        *  \class FastDelegate
