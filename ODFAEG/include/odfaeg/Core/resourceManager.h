...
namespace odfaeg
{
    namespace core {
        ...
        */
        ...
        class ResourceManager : public ResourceManagerBase
        {
            public:
                ...
                */
                ...
                void                      fromFileWithAlias(const std::string& path, const I& alias, A... args, std::tuple<std::reference_wrapper<RA>...> rArgs = std::tuple<std::reference_wrapper<RA>...>());
                 /** \fn  fromFile(const std::string& path)
                *   \brief load a resource from a file.
                *   \param the location of the file. (Relative path)
                */
                ...
                */
                ...
                std::unique_ptr<R> createResourceFromTuple(std::tuple<std::reference_wrapper<RA>...> rArgs, std::index_sequence<Inds...> seq);
                private :
                /**
                * \file ResourceManager.h
                * \class Resource
        ...
        template <typename... RA, size_t... Inds>
        std::unique_ptr<R> ResourceManager<R, I>::createResourceFromTuple(std::tuple<std::reference_wrapper<RA>...> rArgs, std::index_sequence<Inds...> seq) {
            std::unique_ptr<R> resource = std::make_unique<R>(std::get<Inds>(rArgs).get()...);
