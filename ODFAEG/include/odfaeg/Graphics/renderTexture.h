...
namespace odfaeg {
    namespace graphic {
        ...
        public :
            ...
            const int getSwapchainImagesSize();
            const Texture& getTexture(unsigned int currentFrame) const;
            ...
            void submit (bool lastSubmit = false, std::vector<VkSemaphore> signalSemaphores = std::vector<VkSemaphore>(),
                         ...
                         std::vector<uint64_t> waitValues = std::vector<uint64_t>(), std::vector<VkFence> fences = std::vector<VkFence> (), unsigned int queueIndex = 0, bool resetFence = true);
            void display();
        ...
        private :
            ...
            std::array<uint32_t, NB_SWAPCHAIN_IMAGES> value;
            std::vector<VkFence> inFlightFences;
            std::vector<VkSemaphore> imageAvailableSemaphores;
            bool isCubeMap, firstSubmit;
        };
        #else
        namespace priv {
            class RenderTextureImpl;
