            ...
            public :
                ...
                */
                ...
                void scaleEntity(Entity* entity, float x, float y, float z) {
                    if (currentEntityManager != nullptr) {
                        currentEntityManager->scaleEntity(entity, x, y, z);
                    }
                }
                void rotateEntity(Entity* entity, float angle, math::Vec3f axis = math::Vec3f::zAxis) {
                    if (currentEntityManager != nullptr) {
                        currentEntityManager->rotateEntity(entity, angle, axis);
                    }
                }
                ...
                */
                ...
                bool collide(Entity* entity, math::Vec3f position, physic::CollisionResultSet& cinfos) {
                    if (currentEntityManager != nullptr) {
                        return currentEntityManager->collide(entity, position, cinfos);
                    }
                    return false;
                }
                bool collide(Entity* entity, math::Ray ray, physic::CollisionResultSet& cinfos) {
                    if (currentEntityManager != nullptr) {
                        return currentEntityManager->collide(entity, ray, cinfos);
                    }
                    return false;
                }
                ...
                */
                void generate_map(std::vector<Tile*> tGround, std::vector<g2d::Wall*> tWall, math::Vec2f tileSize, physic::BoundingBox zone, EntityFactory& factory) {

                    if (currentEntityManager != nullptr) {
                        currentEntityManager->generate_map(tGround, tWall, tileSize, zone, factory);
                    }
                }
                void generate_3d_map(std::vector<Tile*> tGround, std::vector<g3d::Wall*> tWall, math::Vec2f tileSize, physic::BoundingBox zone, EntityFactory& factory) {

                    if (currentEntityManager != nullptr) {
                        currentEntityManager->generate_3d_map(tGround, tWall, tileSize, zone, factory);
                    }
                }
                void generate_labyrinthe(std::vector<Tile*> tGround, std::vector<g2d::Wall*> tWall, math::Vec2f tileSize, physic::BoundingBox zone, EntityFactory& factory) {

                    if (currentEntityManager != nullptr) {
                        currentEntityManager->generate_labyrinthe(tGround, tWall, tileSize, zone, factory);
                    }
                }
                void generate_3d_labyrinthe(std::vector<Tile*> tGround, std::vector<g3d::Wall*> tWall, math::Vec2f tileSize, physic::BoundingBox zone, EntityFactory& factory) {

                    if (currentEntityManager != nullptr) {
                        currentEntityManager->generate_3d_labyrinthe(tGround, tWall, tileSize, zone, factory);
                    }
                }
                ...
                std::vector<math::Vec2f> getPath(Entity* entity, math::Vec2f finalPos) {
                    ...
                    if (currentEntityManager != nullptr) {
                        if (getGridCellAt(finalPos) != nullptr) {
                            ...
                            while(collide(entity, finalPos, cinfos) && cpt < 1000) {
                                ...
                                for (int i = -50; i <= 50; i+= 50) {
                                    for (int j = -50; j <= 50; j+= 50) {
                                        if (i != 0 && j != 0) {
                                            ...
                                            physic::CollisionResultSet cinfos;
                                            if (!collide(entity, currentPos + v, cinfos) && !visited) {
                ...
                */
                ...
                void addWorker(core::EntitySystem *eu) {
                    ...
                    ptr.reset(eu);
                    cache.eus.emplace_back(std::move(ptr));
                }
                void addTimer(core::Timer *au) {
                    std::unique_ptr<core::Timer> ptr;
                    ptr.reset(au);
                    cache.aus.emplace_back(std::move(ptr));
                }
