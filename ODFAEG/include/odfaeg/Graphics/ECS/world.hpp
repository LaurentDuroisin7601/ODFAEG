...
namespace odfaeg {
    namespace graphic {
        namespace ecs {
            class ODFAEG_GRAPHICS_API World {
                public :
                struct Cache {
                ...
                World(ComponentMapping& componentMapping) : componentMapping(componentMapping) {
                    /*nbEntities = 0;
                    nbEntitiesTypes = 0;*/
                    EntityId moveSystemId = componentMapping.getEntityFactory().createEntity("E_SYSTEM");
                    EntityId resizeSystemId = componentMapping.getEntityFactory().createEntity("E_SYSTEM");
                    EntityId rotationSystemId = componentMapping.getEntityFactory().createEntity("E_SYSTEM");
                    systemQueueIds.push_back(moveSystemId);
                    systemQueueIds.push_back(resizeSystemId);
                    systemQueueIds.push_back(rotationSystemId);
                    MoveSystem mv;
                    ResizeSystem rz;
                    RotationSystem rt;
                    componentMapping.addComponent(moveSystemId, mv);
                    componentMapping.addComponent(resizeSystemId, rz);
                    componentMapping.addComponent(rotationSystemId, rt);
                ...
                void addSystem ( System system, int queue, EntityFactory& factory) {
                    EntityId systemId;
                    if (queue >= systemQueueIds.size()) {
                        systemId = factory.createEntity("E_SYSTEM");
                        systemQueueIds.push_back(systemId);
                    } else {
                        systemId = systemQueueIds[queue];
                    }
                    componentMapping.addComponent(systemId, system, factory);
                }
                ...
                */
                std::vector<graphic::ecs::EntityId> getVisibleEntities (std::string expression) {
                    if (currentEntityManager != nullptr)
                ...
                void moveEntity(EntityId entity, float x, float y, float z){
                    if (currentEntityManager != nullptr) {
                       ...
                       /*if (MoveSystemQueueIndex < systemQueueIds.size()) {
                           EntityId moveSystemId = systemQueueIds[MoveSystemQueueIndex];
                           std::vector<EntityId> entities;
                           entities.push_back(entity);
                           std::tuple<Systems...> sTuple;
                           std::tuple<math::Vec3f> params = std::make_tuple(math::Vec3f(x, y, z));
                           MainSystem mainSystem;
                           mainSystem.template operator()<0, TransformComponent>(moveSystemId, componentMapping, entities, sTuple, params);
                       }*/
                    }
                }
                template <typename... Systems, size_t... I>
                std::tuple<Systems...> toTuple(EntityId entity, std::index_sequence<I...> seq) {
                    auto tuple = std::make_tuple(*componentMapping.getComponent<std::tuple_element_t<I, std::tuple<Systems...>>>(entity)...);
                    return tuple;
                }
                ...
                */
                void generate_map(std::vector<EntityId> tGround, std::vector<EntityId> tWall, math::Vec2f tileSize, physic::BoundingBox zone) {

                    if (currentEntityManager != nullptr) {
                        currentEntityManager->generate_map(tGround, tWall, tileSize, zone);
                    }
                }
                /** \fn void drawOnComponents(std::string expression, int layer, BlendMode mode = BlendMode::BlendAlpha)
                ...
                */
                void drawOnComponents(std::string expression, int layer, BlendMode mode = BlendAlpha) {
                    if (currentEntityManager != nullptr) {
                        currentEntityManager->drawOnComponents(expression, layer, mode);
                    }
                }
                 /** \fn void drawOnComponents(Drawable &drawable, int layer, BlendMode mode = BlendMode::BlendAlpha)
                ...
                std::vector<math::Vec2f> getPath(EntityId entity, math::Vec2f finalPos) {
                    ...
                    if (currentEntityManager != nullptr) {
                        if (getGridCellAt(finalPos) != nullptr) {
                            ...
                            unsigned int cpt = 0;
                            math::Vec2f currentPos = componentMapping.getComponent<TransformComponent>(entity)->center;
            ...
            private :
                ...
                std::vector<EntityId> systemQueueIds;
            };
        }
    }
}
