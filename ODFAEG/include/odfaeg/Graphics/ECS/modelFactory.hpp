...
namespace odfaeg {
    namespace graphic {
        ...
        class ODFAEG_GRAPHICS_API ModelFactory {
                public :
                static EntityId createTileModel(ComponentMapping& componentMapping, const Texture *image, math::Vec3f position, math::Vec3f size, IntRect subRect,  Color color = Color::White) {
                    ...
                    tc.size = size;
                    MeshComponent mesh;
                    VertexArray va(Quads, 4, nullptr, tile);
                    Vertex v1(math::Vec3f(0, 0, 0), color);
                    Vertex v2(math::Vec3f(size.x(), 0, 0), color);
                    Vertex v3(math::Vec3f(size.x(), size.y(), 0), color);
                    Vertex v4(math::Vec3f(0, size.y(), 0), color);
                    ...
                    va[3] = v4;
                    Material material;
                    material.addTexture(image, IntRect(0, 0, 0, 0));
