...
#include <vector>
#include <iostream>
#include "../../../../include/odfaeg/Window/videoMode.hpp"
...
namespace odfaeg {
    namespace window {
        ...
        ////////////////////////////////////////////////////////////
        core::String getErrorString(DWORD errorCode)
        {
        ...
        ////////////////////////////////////////////////////////////
        void WglContext::create(WindowHandle owner,  const ContextSettings& settings, IContext* shared, unsigned int bitsPerPixel)
        {
            // Save the creation settings
            m_settings = settings;
            ...
            createSurface((HWND) owner, bitsPerPixel);
        ...
        bool WglContext::setActive(bool current)
        {
            ...
            {
                std::cerr << "Failed to " << (current ? "activate" : "deactivate") << " OpenGL context: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                return false;
            }
        ...
        void WglContext::setVerticalSyncEnabled(bool enabled)
        {
            ...
            if (sfwgl_ext_EXT_swap_control == sfwgl_LOAD_SUCCEEDED)
            {
                if (wglSwapIntervalEXT(enabled ? 1 : 0) == FALSE)
                    std::cerr << "Setting vertical sync failed: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
            }
            else
            {
                ...
                if (!warned)
                {
                    ...
                    std::cerr << "Setting vertical sync not supported" << std::endl;
        ...
        int WglContext::selectBestPixelFormat(HDC deviceContext, unsigned int bitsPerPixel, const ContextSettings& settings, bool pbuffer)
        {
            ...
            if (sfwgl_ext_ARB_pixel_format == sfwgl_LOAD_SUCCEEDED)
            {
                ...
                if (!isValid)
                    std::cerr << "Failed to enumerate pixel formats: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                ...
                if (isValid && (nbFormats > 0))
                {
                    ...
                    for (UINT i = 0; i < nbFormats; ++i)
                    {
                        ...
                        {
                            std::cerr << "Failed to retrieve pixel format information: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                            break;
                        }
                        ...
                        if (sfwgl_ext_ARB_multisample == sfwgl_LOAD_SUCCEEDED)
                        {
                            ...
                            {
                                std::cerr << "Failed to retrieve pixel format multisampling information: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                                break;
                            }
                        }
                        ...
                        if ((sfwgl_ext_ARB_framebuffer_sRGB == sfwgl_LOAD_SUCCEEDED) || (sfwgl_ext_EXT_framebuffer_sRGB == sfwgl_LOAD_SUCCEEDED))
                        {
                            ...
                            {
                                std::cerr << "Failed to retrieve pixel format sRGB capability information: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                                break;
                            }
                        }
                        ...
                        if (pbuffer)
                        {
                            ...
                            {
                                std::cerr << "Failed to retrieve pixel format pbuffer information: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                                break;
                            }
        ...
        void WglContext::setDevicePixelFormat(unsigned int bitsPerPixel)
        {
            ...
            if (bestFormat == 0)
            {
                std::cerr << "Failed to find a suitable pixel format for device context: " << getErrorString(GetLastError()).toAnsiString() << std::endl
            ...
            {
                std::cerr << "Failed to set pixel format for device context: " << getErrorString(GetLastError()).toAnsiString() << std::endl
                      << "Cannot create OpenGL context" << std::endl;
                return;
            }
        }
        ...
        void WglContext::updateSettingsFromPixelFormat()
        {
            ...
            if (format == 0)
            {
                std::cerr << "Failed to get selected pixel format: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                return;
            }
            ...
            if (shared && shared->m_deviceContext && (sfwgl_ext_ARB_pbuffer == sfwgl_LOAD_SUCCEEDED))
            {
                ...
                if (bestFormat > 0)
                {
                    ...
                    if (m_pbuffer)
                    {
                        ...
                        if (!m_deviceContext)
                        {
                            std::cerr << "Failed to retrieve pixel buffer device context: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                    ...
                    {
                        std::cerr << "Failed to create pixel buffer: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                    }
                }
            }
        ...
        void WglContext::createContext(WglContext* shared)
        {
                ...
                if (sfwgl_ext_ARB_create_context == sfwgl_LOAD_SUCCEEDED)
                {
                    ...
                    else
                    {
                        if ((m_settings.attributeFlags & ContextSettings::Core) || (m_settings.attributeFlags & ContextSettings::Debug))
                            std::cerr << "Selecting a profile during context creation is not supported,"
                    ...
                    if (sharedContext)
                    {
                        ...
                        if (currentContext == shared)
                        {
                            ...
                            {
                                std::cerr << "Failed to deactivate shared context before sharing: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                                return;
                            }
            ...
            if (!m_context)
            {
                ...
                if (!m_context)
                {
                    std::cerr << "Failed to create an OpenGL context for this window: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                    return;
                }
                ...
                if (sharedContext)
                {
                    ...
                    if (wglShareLists(sharedContext, m_context) == FALSE)
                        std::cerr << "Failed to share the OpenGL context: " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                }
            }
        }
    }
