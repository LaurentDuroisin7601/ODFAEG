...
namespace odfaeg {
    namespace graphic {
        namespace ecs {
            Cell::Cell (physic::BoundingPolyhedron bp, math::Vec3f coords, ComponentMapping& componentMapping) : componentMapping(componentMapping) {
                cellVolume = bp;
                passable = true;
                stateChanged = false;
                traveled = false;
            ...
            bool Cell::removeEntity (std::string type) {
                typename vector<EntityId>::iterator it;
                for (it = entitiesInside.begin(); it != entitiesInside.end();) {
                    if (componentMapping.getComponent<EntityInfoComponent>(*it)->groupName == type) {
                        it = entitiesInside.erase(it);
                        return true;
                    } else
                        it++;
