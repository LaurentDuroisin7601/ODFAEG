            ...
            LightRenderComponent::LightRenderComponent (RenderWindow& window, int layer, std::string expression, ComponentMapping& componentMapping, window::ContextSettings settings) :
                        ...
                        lightMapGenerator.setParameter("lightMap",lightMap.getTexture());
                            ...
                            if (Shader::isAvailable()) {
                                ...
                                "void main() {"
                                    "vec4 depth = texture2D(texture, gl_TexCoord[0].x()y);"
                                    "float s01 = textureOffset(texture, gl_TexCoord[0].x()y, off.x()y).z();"
                                    "float s21 = textureOffset(texture, gl_TexCoord[0].x()y, off.z()y).z();"
                                    "float s10 = textureOffset(texture, gl_TexCoord[0].x()y, off.y()x).z();"
                                    "float s12 = textureOffset(texture, gl_TexCoord[0].x()y, off.y()z).z();"
                                    "vec3 va = normalize (vec3(size.x()y, s21 - s01));"
                                    "vec3 vb = normalize (vec3(size.y()x, s12 - s10));"
                            ...
                            "}"
                                 "void main () { "
                                     ...
                                     "vec3 vertexToLight = sLightPos - pixPos;"
                                     "if (bump.x() != 0 || bump.y() != 0 || bump.z() != 0) {"
                                         "float s01 = textureOffset(normalMap, position, off.x()y).z();"
                                         "float s21 = textureOffset(normalMap, position, off.z()y).z();"
                                         "float s10 = textureOffset(normalMap, position, off.y()x).z();"
                                         "float s12 = textureOffset(normalMap, position, off.y()z).z();"
                                         "vec3 tmpNormal = (normal.x()yz);"
                                         "vec3 tangeant = normalize (vec3(size.x()y, s21 - s01));"
                                         "vec3 binomial = normalize (vec3(size.y()x, s12 - s10));"
                                     ...
                                     "if (z >= normal.w) {"
                                         ...
                                         "if (normal.x() != 0 || normal.y() != 0 || normal.z() != 0 && vertexToLight.z() > 0.f) {"
                                             "vec3 dirToLight = normalize(vertexToLight.x()yz);"
                                             "float nDotl = dot (dirToLight, normal.x()yz);"
                                             "attenuation *= nDotl;"
                                         "}"
                                         "gl_FragColor = vec4(lightColor.x()yz, 1) * max(0.0f, attenuation) + specularColor;"
                                     "} else {"
