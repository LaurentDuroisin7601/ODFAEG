...
namespace odfaeg {
    namespace graphic {
        #ifdef VULKAN
        PerPixelLinkedListRenderComponent2::PerPixelLinkedListRenderComponent2(RenderWindow& window, int layer, std::string expression, window::ContextSettings settings, bool useThread) :
            ...
            /*threadPoolFences.resize(MAX_FRAMES_IN_FLIGHT);
            for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
                if (vkCreateFence(vkDevice.getDevice(), &fenceInfo, nullptr, &threadPoolFences[i]) != VK_SUCCESS) {
                    throw std::runtime_error("failed to create fence");
                }
            }*/
            ...
            for (unsigned int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
                jobFence[i].reset(numThreads);
            }


        }
        void PerPixelLinkedListRenderComponent2::createDescriptorsAndPipelines() {
            RenderStates states;
            if (useThread) {
        ...
        void PerPixelLinkedListRenderComponent2::launchRenderer () {
            if (useThread) {
                stop = false;
                /*for (unsigned int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
                ...
                executePools = std::thread(executeThreadPools, this);
                getListener().launch();
        ...
        void PerPixelLinkedListRenderComponent2::stopRenderer() {
            //std::cout<<"stop!"<<std::endl;

            stop = true;
            cv.notify_all();
            cv2.notify_all();
            executeThreadPoolRunning = false;
            executePools.join();
            getListener().stop();
        ...
        uint32_t PerPixelLinkedListRenderComponent2::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
        ...
        void PerPixelLinkedListRenderComponent2::createHeadPtrImageView() {
            for (unsigned int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        void PerPixelLinkedListRenderComponent2::createHeadPtrSampler() {
        ...
        void PerPixelLinkedListRenderComponent2::resetBuffers() {
            for (unsigned int i = 0; i < Batcher::nbPrimitiveTypes; i++) {
        ...
        VkCommandBuffer PerPixelLinkedListRenderComponent2::beginSingleTimeCommands() {
        ...
        void PerPixelLinkedListRenderComponent2::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
        ...
        void PerPixelLinkedListRenderComponent2::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
        ...
        void PerPixelLinkedListRenderComponent2::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
            VkCommandBuffer commandBuffer = beginSingleTimeCommands();
        ...
        unsigned int PerPixelLinkedListRenderComponent2::align(unsigned int offset) {
            ////std::cout << "alignment = " << alignment << std::endl;
            return (offset + alignment - 1) & ~(alignment - 1);
        }
        void PerPixelLinkedListRenderComponent2::createDescriptorPool(unsigned int p, RenderStates states) {
        ...
        void PerPixelLinkedListRenderComponent2::createDescriptorPool(RenderStates states) {
        ...
        void PerPixelLinkedListRenderComponent2::createDescriptorSetLayout(RenderStates states) {
        ...
        void PerPixelLinkedListRenderComponent2::allocateDescriptorSets(unsigned int p, RenderStates states) {
        ...
        void PerPixelLinkedListRenderComponent2::allocateDescriptorSets(RenderStates states) {
            /*std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets = frameBuffer.getDescriptorSet();
            std::vector<std::vector<VkDescriptorPool>>& descriptorPool = frameBuffer.getDescriptorPool();
            std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = frameBuffer.getDescriptorSetLayout();
            Shader* shader = const_cast<Shader*>(states.shader);
            ...
            variableCountInfo.descriptorSetCount = static_cast<uint32_t>(variableCounts.size());;
            variableCountInfo.pDescriptorCounts = variableCounts.data();
            std::vector<VkDescriptorSetLayout> layouts(frameBuffer.getMaxFramesInFlight(), descriptorSetLayout[descriptorId][0]);
            VkDescriptorSetAllocateInfo allocInfo{};
            allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
            allocInfo.pNext = &variableCountInfo;
            allocInfo.descriptorPool = descriptorPool[descriptorId][0];
        ...
        void PerPixelLinkedListRenderComponent2::updateDescriptorSets(unsigned int currentFrame, unsigned int p, RenderStates states) {
            std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets = frameBuffer.getDescriptorSet();
            Shader* shader = const_cast<Shader*>(states.shader);
            std::vector<Texture*> allTextures = Texture::getAllTextures();
            if (shader == &indirectRenderingShader) {
        ...
        void PerPixelLinkedListRenderComponent2::createDescriptorSets(unsigned int p, RenderStates states) {
        ...
        void PerPixelLinkedListRenderComponent2::createDescriptorPool2(RenderStates states) {
            /*std::vector<VkDescriptorPool>& descriptorPool = frameBuffer.getDescriptorPool();
            Shader* shader = const_cast<Shader*>(states.shader);
            if (shader->getNbShaders() > descriptorPool.size())
                descriptorPool.resize(shader->getNbShaders());
        ...
        void PerPixelLinkedListRenderComponent2::createDescriptorSetLayout2(RenderStates states) {
            std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = frameBuffer.getDescriptorSetLayout();
            Shader* shader = const_cast<Shader*>(states.shader);
            if (shader->getNbShaders() > descriptorSetLayout.size())
                descriptorSetLayout.resize(shader->getNbShaders());
        ...
        void PerPixelLinkedListRenderComponent2::allocateDescriptorSets2(RenderStates states) {
            /*std::vector<std::vector<VkDescriptorSet>>& descriptorSets = frameBuffer.getDescriptorSet();
        ...
        void PerPixelLinkedListRenderComponent2::createDescriptorSets2(RenderStates states) {
            /*std::vector<std::vector<VkDescriptorSet>>& descriptorSets = frameBuffer.getDescriptorSet();
            Shader* shader = const_cast<Shader*>(states.shader);
        ...
        void PerPixelLinkedListRenderComponent2::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
        ...
        void PerPixelLinkedListRenderComponent2::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
            VkCommandBufferAllocateInfo allocInfo{};
            allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
            allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
            allocInfo.commandPool = commandPool;
        ...
        void PerPixelLinkedListRenderComponent2::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkCommandBuffer cmd) {
        ...
        void PerPixelLinkedListRenderComponent2::fillBuffersMT() {
        ...
        void PerPixelLinkedListRenderComponent2::fillIndexedBuffersMT () {
        ...
        void PerPixelLinkedListRenderComponent2::fillSelectedBuffersMT() {
            std::array<unsigned int, Batcher::nbPrimitiveTypes> firstIndex, baseInstance;
            for (unsigned int i = 0; i < firstIndex.size(); i++) {
                firstIndex[i] = 0;
            }
        ...
        void PerPixelLinkedListRenderComponent2::fillSelectedIndexedBuffersMT() {
            std::array<unsigned int, Batcher::nbPrimitiveTypes> firstIndex, baseInstance, baseVertex;
            for (unsigned int i = 0; i < firstIndex.size(); i++) {
                firstIndex[i] = 0;
            }
        ...
        void PerPixelLinkedListRenderComponent2::fillOutlineBuffersMT() {
            std::array<unsigned int, Batcher::nbPrimitiveTypes> firstIndex, baseInstance;
            for (unsigned int i = 0; i < firstIndex.size(); i++) {
                firstIndex[i] = 0;
            }
        ...
        void PerPixelLinkedListRenderComponent2::fillOutlineIndexedBuffersMT () {
            std::array<unsigned int, Batcher::nbPrimitiveTypes> firstIndex, baseInstance, baseVertex;
            for (unsigned int i = 0; i < firstIndex.size(); i++) {
                firstIndex[i] = 0;
            }
        ...
        void PerPixelLinkedListRenderComponent2::drawInstances() {
            for (unsigned int i = 0; i < Batcher::nbPrimitiveTypes; i++) {
                vbBindlessTex[i].clear();
                materialDatas[i].clear();
                modelDatas[i].clear();
        ...
        void PerPixelLinkedListRenderComponent2::drawInstancesIndexed() {
            for (unsigned int i = 0; i < Batcher::nbPrimitiveTypes; i++) {
                vbBindlessTex[i].clear();
                materialDatas[i].clear();
                modelDatas[i].clear();
        ...
        void PerPixelLinkedListRenderComponent2::drawSelectedInstances() {
            for (unsigned int i = 0; i < Batcher::nbPrimitiveTypes; i++) {
                vbBindlessTex[i].clear();
                materialDatas[i].clear();
                modelDatas[i].clear();
        ...
        void PerPixelLinkedListRenderComponent2::drawSelectedInstancesIndexed() {
            for (unsigned int i = 0; i < Batcher::nbPrimitiveTypes; i++) {
                vbBindlessTex[i].clear();
                materialDatas[i].clear();
                modelDatas[i].clear();
        ...
        void PerPixelLinkedListRenderComponent2::executeThreadPools() {
            ...
            while (executeThreadPoolRunning)    {
                if (isVisible()) {
                    ...
                    cv.wait(lock, [this]{ return offscreenCommandBufferReady[threadpoolCurrentFrame].load() || stop.load();});


                    offscreenCommandBufferReady[threadpoolCurrentFrame] = false;
                    VkSemaphoreWaitInfo waitInfo{};
                    uint64_t waitValue = values2[threadpoolCurrentFrame];
                    //std::cout<<"wait value : "<<waitValue<<std::endl;
                    ...
                    if (!stop.load()) {
                        ...
                        threadPool.enqueue([this, currentFrame, currentStates]{
                            ...
                            //std::cout<<"copy registered  : "<<currentFrame<<std::endl;
                        });
                        drawBuffers();
        ...
        void PerPixelLinkedListRenderComponent2::drawNextFrame() {
            ...
            if (isVisible()) {
                ...
                if (useThread) {
                        ...
                        //std::cout<<"cmd registered : "<<frameBuffer.getCurrentFrame()<<","<<getLayer()<<std::endl;
                        ...
                        cv.wait(lock, [this, currentFrame](){return registerFrameJob[currentFrame].load() || stop.load();});
                                ...
                                if (visibleEntities[i] != nullptr && visibleEntities[i]->isLeaf() && (visibleEntities[i]->getRootType() == "E_PARTICLES"
                                    ...
                                    for (unsigned int p = 0; p < Batcher::nbPrimitiveTypes; p++) {
                                        if (vbBindlessTex[p].getVertexCount() > 0) {
                                            ...
                                            visibleEntities[i]->getRootEntity()->computeParticles(&mtx2, &cv2, vbBindlessTex[p], frameBuffer.getCurrentFrame(),visibleEntities[i]->getTransform(), (visibleEntities[i]->getDrawMode() == Entity::INSTANCED) ? true : false, computeSemaphores[i], computeFences[i], getLayer());
        ...
        void PerPixelLinkedListRenderComponent2::allocateCommandBuffers() {
        ...
        void PerPixelLinkedListRenderComponent2::recordCommandBufferIndirect(unsigned int currentFrame, unsigned int p, unsigned int nbIndirectCommands, unsigned int stride, PPLLDepthStencilID depthStencilID, unsigned int vertexOffset, unsigned int indexOffset, unsigned int uboOffset, unsigned int modelDataOffset, unsigned int materialDataOffset, unsigned int drawCommandOffset, RenderStates currentStates, VkCommandBuffer commandBuffer) {
            //std::lock_guard<std::recursive_mutex> lock(rec_mutex);
        ...
        void PerPixelLinkedListRenderComponent2::recordCommandBufferVertexBuffer(unsigned int currentFrame, RenderStates currentStates, VkCommandBuffer commandBuffer) {
            //std::lock_guard<std::recursive_mutex> lock(rec_mutex);
            currentStates.blendMode.updateIds();
        ...
        void PerPixelLinkedListRenderComponent2::drawBuffers() {
            unsigned int currentFrame = threadpoolCurrentFrame;
            unsigned int texturesInUse = Texture::getAllTextures().size();
            //std::cout<<"current frame : "<<threadpoolCurrentFrame<<std::endl;
            ...
            for (unsigned int p = 0; p < Batcher::nbPrimitiveTypes; p++) {
                ...
                if (bufferSize > 0) {
                    ...
                    if (modelDataStagingBufferMemoryMT[p][currentFrame] != nullptr) {
                        void* data;
                ...
                if (bufferSize > 0) {
                    ...
                    if (materialDataBufferMemoryMT[p][currentFrame] != nullptr) {
                        void* data;
                ...
                if (bufferSize > 0) {
                    ...
                    if (vboIndexedIndirectStagingBufferMemoryMT[p][currentFrame] != nullptr) {
                        void* data;
            ...
            threadPool.enqueue([this, currentFrame, currentStates] {
                ...
                jobFence[currentFrame].jobDone();
                //std::cout<<"skybox registered : "<<currentFrame<<std::endl;
            });
            currentStates.blendMode = BlendNone;
            currentStates.shader = &indirectRenderingShader;
            for (unsigned int p = 0; p < Batcher::nbPrimitiveTypes-1; p++) {
                if (needToUpdateDSs[p][currentFrame]) {
                    //std::cout<<"update ds : "<<threadpoolCurrentFrame<<std::endl;
            ...
            threadPool.enqueue([this, currentFrame, currentStates] {
                ...
                jobFence[currentFrame].jobDone();
                //std::cout<<"indirect registered,"<<currentFrame<<std::endl;
            });
            threadPool.enqueue([this, currentFrame, currentStates]{
                VkCommandBufferInheritanceInfo inheritanceInfo{};
                ...
                jobFence[currentFrame].jobDone();
                //std::cout<<"selected indirect registered,"<<currentFrame<<std::endl;
            });
            ...
            threadPool.enqueue([this, currentFrame, currentStates]{
                ...
                jobFence[currentFrame].jobDone();
                //std::cout<<"outline indirect registered,"<<currentFrame<<std::endl;
            });
            ...
            threadPool.enqueue([this, currentFrame, currentStates] {
                ...
                jobFence[currentFrame].jobDone();
                //std::cout<<"indirect ppl 2 registered,"<<currentFrame<<std::endl;
            });

            for (unsigned int p = 0; p < (Batcher::nbPrimitiveTypes - 1); p++)
                needToUpdateDSs[p][currentFrame] = false;
        }
        void PerPixelLinkedListRenderComponent2::createCommandBuffersIndirect(unsigned int p, unsigned int nbIndirectCommands, unsigned int stride, PPLLDepthStencilID depthStencilID, RenderStates currentStates) {
        ...
        void PerPixelLinkedListRenderComponent2::createCommandBufferVertexBuffer(RenderStates currentStates) {
        ...
        Entity* PerPixelLinkedListRenderComponent2::getBorder(Entity* entity) {
        ...
        bool PerPixelLinkedListRenderComponent2::loadEntitiesOnComponent(std::vector<Entity*> vEntities) {
            {
        ...
        void PerPixelLinkedListRenderComponent2::createCommandPool() {
        ...
        PerPixelLinkedListRenderComponent2::~PerPixelLinkedListRenderComponent2() {
            ...
            vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[0], copyVbPpllPass2CommandBuffer.size(), copyVbPpllPass2CommandBuffer.data());
            ...
            vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[1], ppllCommandBuffer.size(), ppllCommandBuffer.data());
            vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[2], ppllSelectedCommandBuffer.size(), ppllSelectedCommandBuffer.data());
            vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[3], ppllOutlineCommandBuffer.size(), ppllOutlineCommandBuffer.data());
            vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[4], ppllPass2CommandBuffer.size(), ppllPass2CommandBuffer.data());
            vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[5], skyboxCommandBuffer.size(), skyboxCommandBuffer.data());
            for (unsigned int i = 0; i < secondaryBufferCommandPools.size(); i++) {
        ...
        void PerPixelLinkedListRenderComponent2::setBackgroundColor(Color color) {
        ...
        void PerPixelLinkedListRenderComponent2::onVisibilityChanged(bool visible) {
            if (visible) {
                glCheck(glBindBufferBase(GL_UNIFORM_BUFFER, 0, ubo));
                glCheck(glBindBufferBase(GL_ATOMIC_COUNTER_BUFFER, 0, atomicBuffer));
                glCheck(glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, linkedListBuffer));
