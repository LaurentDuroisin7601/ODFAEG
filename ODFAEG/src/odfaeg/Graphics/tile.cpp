    ...
    namespace graphic {
        ...
        */
        ...
        void Tile::changeVerticesHeights(float h1, float h2, float h3, float h4) {
            ...
            getFace(0)->getVertexArray()[1].position[1] = h2;
            getFace(0)->getVertexArray()[2].position[1] = h3;
            getFace(0)->getVertexArray()[3].position[1] = h4;
            float min, max;
        ...
        void Tile::setTexRect(IntRect rect) {
            ...
            va[0].texCoords = math::Vec2f(rect.left, rect.top);
            va[1].texCoords = math::Vec2f(rect.left + rect.width, rect.top);
            va[2].texCoords = math::Vec2f(rect.left + rect.width, rect.top + rect.height);
            va[3].texCoords = math::Vec2f(rect.left, rect.top + rect.height);
        }
    }
}
