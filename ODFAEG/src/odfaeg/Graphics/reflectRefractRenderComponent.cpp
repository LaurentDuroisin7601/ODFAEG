    ...
    namespace graphic {
            ...
            void ReflectRefractRenderComponent::compileShaders() {
                ...
                const std::string fragmentShader = R"(#version 460
                                                      ...
                                                      layout(set = 0, binding = 2) buffer CounterSSBO {
                                                          uint count[6];
                                                          uint maxNodes;
                                                      };
                                                      layout(set = 0, binding = 3, r32ui) uniform coherent uimage2DArray headPointers;
                                                      layout(set = 0, binding = 4) buffer linkedLists {
                                                          NodeType nodes[];
                                                      };
            ...
            void ReflectRefractRenderComponent::createDescriptorSetLayout(RenderStates states) {
                ...
                if (shader == &skyboxShader) {
                    ...
                    counterLayoutBinding.binding = 2;
                    ...
                    linkedListLayoutBinding.binding = 4;
            ...
            void ReflectRefractRenderComponent::updateDescriptorSets(unsigned int currentFrame, unsigned int p, RenderStates states) {
                ...
                if (shader == &skyboxShader) {
                    ...
                    descriptorWrites[2].pBufferInfo = &counterStorageBufferInfoLastFrame;
                    ...
                    descriptorWrites[4].pBufferInfo = &linkedListStorageBufferInfoLastFrame;
            ...
            void ReflectRefractRenderComponent::drawNextFrame() {
                ...
                if (useThread) {
                    ...
                    alphaBuffer.submit(true, signalSemaphores, waitSemaphores, waitStages,std::vector<uint64_t>(), std::vector<uint64_t>(), std::vector<VkFence>(), getLayer()+2);
                    ...
                    for (unsigned int i = 0; i < environmentMapCommandBuffer.size(); i++) {
                        ...
                        signalValues.clear();
                        signalValues.push_back(values[reflectRefractTex.getCurrentFrame()]);
                        environmentMap.submit(true, signalSemaphores, waitSemaphores, waitStages, signalValues, waitValues, std::vector<VkFence>(), getLayer()+2);
                        ...
                        signalValues.clear();
                        signalValues.push_back(values[currentFrame]);
                        reflectRefractTex.submit((i == nbReflRefrEntities - 1) ? true : false, signalSemaphores, waitSemaphores, waitStages, signalValues, waitValues, std::vector<VkFence>(),getLayer()+2);
                    ...
                    if (!isSomethingDrawn) {
                        ...
                        signalValues.clear();
                        signalValues.push_back(values[reflectRefractTex.getCurrentFrame()]);
                        reflectRefractTex.submit(true, signalSemaphores, waitSemaphores, waitStages, signalValues, std::vector<uint64_t>(), std::vector<VkFence>(), 3);
