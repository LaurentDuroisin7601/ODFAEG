        ...
        namespace g3d {
            ...
            void PonctualLight::initTriangles() {
                ...
                for (int i = 0; i < end; i++) {
                    Vector3f v1, v2, v3;
                    v1 = Vector3f (0, 0, 0);
                    v2 = Vector3f (getSize().x * 0.5f * math::Math::cosinus (i * angle), 0, getSize().z * 0.5f * math::Math::sinus (i * angle));
                    v3 = Vector3f (getSize().x * 0.5f * math::Math::cosinus ((i + 1) * angle), 0, getSize().z * 0.5f * math::Math::sinus ((i + 1) * angle));
                    VertexArray *triangle = new VertexArray (Triangles, 3, this);
                    (*triangle)[0] = Vertex(v1, color);
                    (*triangle)[1] = Vertex(v2, color);
                    (*triangle)[2] = Vertex(v3, color);
