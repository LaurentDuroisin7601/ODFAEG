    ...
    namespace graphic {
        ...
        VkPresentModeKHR RenderWindow::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
            for (const auto& availablePresentMode : availablePresentModes) {
                if (!isVerticalSynchEnabled() && availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
        ...
        void RenderWindow::registerClearCommands(const Color& color) {
             ...
             //for (unsigned int i = 0; i < getCommandBuffers().size(); i++) {
                ...
                vkCmdPipelineBarrier(getCommandBuffers()[currentFrame], VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &clearToPresentBarrier);
        ...
        void RenderWindow::submit(bool lastSubmit, std::vector<VkSemaphore> signalSemaphores,
            ...
            if (getCommandBuffers().size() > 0) {
                ...
                if (vkQueueSubmit(vkDevice.getQueue(indices.graphicsFamily.value(), queueIndex), 1, &submitInfo, (fenceToSubmit == nullptr) ? inFlightFences[currentFrame] : fenceToSubmit) != VK_SUCCESS) {
                    throw core::Erreur(0, "échec de l'envoi d'un command buffer window!", 1);
                }
