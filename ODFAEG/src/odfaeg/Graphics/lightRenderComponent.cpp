...
namespace odfaeg {
    namespace graphic {
        ...
        LightRenderComponent::LightRenderComponent (RenderWindow& window, int layer, std::string expression,window::ContextSettings settings, bool useThread) :
                ...
                bufferAllocInfo.commandPool = secondaryBufferCommandPools[5];
                if (vkAllocateCommandBuffers(vkDevice.getDevice(), &bufferAllocInfo, bumpCommandBuffer.data()) != VK_SUCCESS) {
                    throw core::Erreur(0, "failed to allocate command buffers!", 1);
                }
                bufferAllocInfo.commandPool = secondaryBufferCommandPools[6];
            ...
            void LightRenderComponent::compileShaders() {
                const std::string indirectRenderingVertexShader = R"(#version 460
                                                                             ...
                                                                             void main() {
                                                                                 ...
                                                                                 gl_Position = pushConsts.projectionMatrix * pushConsts.viewMatrix * modelMatrix * vec4(position, 1.f);
                        ...
                        const std::string perPixLightingIndirectRenderingVertexShader = R"(#version 460
                                                                                      ...
                                                                                      layout(binding = 6) buffer objectData {
                                                                                         ModelData objectDatas[];
                                                                                      };
                                                                                      layout (location = 0) out vec2 fTexCoords;
                                                                                      layout (location = 1) out vec4 frontColor;
                        ...
                        const std::string buildAlphaBufferFragmentShader = R"(#version 460
                                                                      ...
                                                                      layout(binding = 3, rgba32f) uniform coherent image2D alphaBuffer;
            ...
            void LightRenderComponent::launchRenderer() {
                if (useThread) {
                    stop = false;
                    registerFrameJob[lightMap.getCurrentFrame()] = true;
                    commandBufferReady[lightMap.getCurrentFrame()] = false;
                    getListener().launch();
                }
            }
            ...
            void LightRenderComponent::createDescriptorPool(unsigned int p, RenderStates states) {
                ...
                if (shader == &depthBufferGenerator) {
                    ...
                    {
                        std::vector<std::vector<VkDescriptorPool>>& descriptorPool = lightDepthBuffer.getDescriptorPool();
                        if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorPool.size())
                            descriptorPool.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    poolSizes[3].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                    poolSizes[3].descriptorCount = static_cast<uint32_t>(alphaBuffer.getMaxFramesInFlight() * MAX_TEXTURES);
                    poolSizes[4].type = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
                    poolSizes[4].descriptorCount = static_cast<uint32_t>(alphaBuffer.getMaxFramesInFlight());
                    poolSizes[5].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                    poolSizes[5].descriptorCount = static_cast<uint32_t>(alphaBuffer.getMaxFramesInFlight()) * lightDepthBuffer.getSwapchainImagesSize();
                    ...
                    std::vector<std::vector<VkDescriptorPool>>& descriptorPool = specularTexture.getDescriptorPool();
                    if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorPool.size())
                        descriptorPool.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    poolSizes[3].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                    poolSizes[3].descriptorCount = static_cast<uint32_t>(specularTexture.getMaxFramesInFlight() * MAX_TEXTURES);
                    poolSizes[4].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                    poolSizes[4].descriptorCount = static_cast<uint32_t>(specularTexture.getMaxFramesInFlight()) * depthBuffer.getSwapchainImagesSize();
                    if (descriptorPool[descriptorId][0] != nullptr) {
                        vkDestroyDescriptorPool(vkDevice.getDevice(), descriptorPool[descriptorId][0], nullptr);
                    }
                    ...
                    std::vector<std::vector<VkDescriptorPool>>& descriptorPool = bumpTexture.getDescriptorPool();
                    if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorPool.size())
                        descriptorPool.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    poolSizes[3].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                    poolSizes[3].descriptorCount = static_cast<uint32_t>(bumpTexture.getMaxFramesInFlight() * MAX_TEXTURES);
                    poolSizes[4].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                    poolSizes[4].descriptorCount = static_cast<uint32_t>(bumpTexture.getMaxFramesInFlight()) * depthBuffer.getSwapchainImagesSize();
                    if (descriptorPool[descriptorId][0] != nullptr) {
                        vkDestroyDescriptorPool(vkDevice.getDevice(), descriptorPool[descriptorId][0], nullptr);
                    }
                    ...
                    std::vector<std::vector<VkDescriptorPool>>& descriptorPool = lightMap.getDescriptorPool();
                    if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorPool.size())
                        descriptorPool.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
            ...
            void LightRenderComponent::createDescriptorSetLayout(RenderStates states) {
                ...
                if (shader == &depthBufferGenerator) {
                    ...
                    {
                        std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = lightDepthBuffer.getDescriptorSetLayout();
                        if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorSetLayout.size())
                            descriptorSetLayout.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    std::array<VkDescriptorSetLayoutBinding, 6> bindings = {modelDataLayoutBinding, materialDataLayoutBinding, objectDataLayoutBinding, headPtrImageLayoutBinding, samplerLayoutBinding2, samplerLayoutBinding};
                    ...
                    std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = specularTexture.getDescriptorSetLayout();
                    if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorSetLayout.size())
                        descriptorSetLayout.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    std::array<VkDescriptorSetLayoutBinding, 5> bindings = {modelDataLayoutBinding, materialDataLayoutBinding, objectDataLayoutBinding, samplerLayoutBinding2, samplerLayoutBinding};
                    ...
                    std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = bumpTexture.getDescriptorSetLayout();
                    if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorSetLayout.size())
                        descriptorSetLayout.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = lightMap.getDescriptorSetLayout();
                    if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorSetLayout.size())
                        descriptorSetLayout.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                    ...
                    VkDescriptorSetLayoutBinding objectDataLayoutBinding{};
                    objectDataLayoutBinding.binding = 6;
                    objectDataLayoutBinding.descriptorCount = 1;
                    objectDataLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
                    objectDataLayoutBinding.pImmutableSamplers = nullptr;
                    objectDataLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
                    ...
                    std::array<VkDescriptorSetLayoutBinding, 7> bindings = {samplerLayoutBinding, samplerLayoutBinding2, samplerLayoutBinding3, samplerLayoutBinding4, modelDataLayoutBinding, materialDataLayoutBinding, objectDataLayoutBinding};
            ...
            void LightRenderComponent::updateDescriptorSets(unsigned int currentFrame, unsigned int p, RenderStates states, bool lightDepth) {
                Shader* shader = const_cast<Shader*>(states.shader);
                if (shader == &depthBufferGenerator) {
                    std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets = (lightDepth) ? lightDepthBuffer.getDescriptorSet() : depthBuffer.getDescriptorSet();
                    std::vector<Texture*> allTextures = Texture::getAllTextures();
                    unsigned int descriptorId = p * shader->getNbShaders() + shader->getId();
                    ...
                    std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets = specularTexture.getDescriptorSet();
                    std::vector<Texture*> allTextures = Texture::getAllTextures();
                    unsigned int descriptorId = p * shader->getNbShaders() + shader->getId();
                    std::vector<VkDescriptorImageInfo>	descriptorImageInfos;
                    descriptorImageInfos.resize(allTextures.size());
                    ...
                    std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets = bumpTexture.getDescriptorSet();
                    std::vector<Texture*> allTextures = Texture::getAllTextures();
                    unsigned int descriptorId = p * shader->getNbShaders() + shader->getId();
                    std::array<VkWriteDescriptorSet, 5> descriptorWrites{};
                    ...
                    std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets = lightMap.getDescriptorSet();
                    unsigned int descriptorId = p * shader->getNbShaders() + shader->getId();
                    ...
                    descriptorWrites[6].pBufferInfo = &objectDataStorageBufferInfoLastFrame;
            ...
            void LightRenderComponent::allocateDescriptorSets(unsigned int p, RenderStates states) {
                ...
                if (shader == &depthBufferGenerator) {
                    ...
                    {
                        std::vector<std::vector<std::vector<VkDescriptorSet>>>& descriptorSets =     lightDepthBuffer.getDescriptorSet();
                        std::vector<std::vector<VkDescriptorPool>>& descriptorPool = lightDepthBuffer.getDescriptorPool();
                        std::vector<std::vector<VkDescriptorSetLayout>>& descriptorSetLayout = lightDepthBuffer.getDescriptorSetLayout();
                        if (shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1) > descriptorSets.size())
                            descriptorSets.resize(shader->getNbShaders() * (Batcher::nbPrimitiveTypes - 1));
                        ...
                        descriptorSets[descriptorId].resize(1);
                        for (unsigned int i = 0; i < descriptorSets[descriptorId].size(); i++) {
                            descriptorSets[descriptorId][i].resize(lightDepthBuffer.getMaxFramesInFlight());
                        }
                        std::vector<Texture*> allTextures = Texture::getAllTextures();
                        std::vector<uint32_t> variableCounts(depthBuffer.getMaxFramesInFlight(), static_cast<uint32_t>(MAX_TEXTURES));
                    ...
                    descriptorSets[descriptorId].resize(1);
                    for (unsigned int i = 0; i < descriptorSets[descriptorId].size(); i++) {
                        descriptorSets[descriptorId][i].resize(specularTexture.getMaxFramesInFlight());
                    }
                    std::vector<Texture*> allTextures = Texture::getAllTextures();
                    std::vector<uint32_t> variableCounts(depthBuffer.getMaxFramesInFlight(), static_cast<uint32_t>(MAX_TEXTURES));
                    ...
                    variableCountInfo.descriptorSetCount = static_cast<uint32_t>(variableCounts.size());
                    variableCountInfo.pDescriptorCounts = variableCounts.data();
                    std::vector<VkDescriptorSetLayout> layouts(specularTexture.getMaxFramesInFlight(), descriptorSetLayout[shader->getId()][0]);
                    VkDescriptorSetAllocateInfo allocInfo{};
                    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
                    allocInfo.pNext = &variableCountInfo;
                    allocInfo.descriptorPool = descriptorPool[descriptorId][0];
                    ...
                    descriptorSets[descriptorId].resize(1);
                    for (unsigned int i = 0; i < descriptorSets[descriptorId].size(); i++) {
                        descriptorSets[descriptorId][i].resize(bumpTexture.getMaxFramesInFlight());
                    }
                    std::vector<Texture*> allTextures = Texture::getAllTextures();
                    std::vector<uint32_t> variableCounts(depthBuffer.getMaxFramesInFlight(), static_cast<uint32_t>(MAX_TEXTURES));
                    ...
                    variableCountInfo.descriptorSetCount = static_cast<uint32_t>(variableCounts.size());
                    variableCountInfo.pDescriptorCounts = variableCounts.data();
                    std::vector<VkDescriptorSetLayout> layouts(bumpTexture.getMaxFramesInFlight(), descriptorSetLayout[shader->getId()][0]);
                    VkDescriptorSetAllocateInfo allocInfo{};
                    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
                    allocInfo.pNext = &variableCountInfo;
                    allocInfo.descriptorPool = descriptorPool[descriptorId][0];
                    ...
                    descriptorSets[descriptorId].resize(1);
                    for (unsigned int i = 0; i < descriptorSets[descriptorId].size(); i++) {
                        descriptorSets[descriptorId][i].resize(lightMap.getMaxFramesInFlight());
                    }
                    std::vector<VkDescriptorSetLayout> layouts(lightMap.getMaxFramesInFlight(), descriptorSetLayout[shader->getId()][0]);
                    VkDescriptorSetAllocateInfo allocInfo{};
                    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
                    allocInfo.descriptorPool = descriptorPool[descriptorId][0];
        ...
        void LightRenderComponent::fillLightBuffersMT() {
            ...
            for (unsigned int i = 0; i < m_light_instances.size(); i++) {
                if (m_light_instances[i].getAllVertices().getVertexCount() > 0) {
                    ...
                    unsigned int vertexCount = 0;
                    for (unsigned int j = 0; j < m_light_instances[i].getAllVertices().getVertexCount(); j++) {
                        m_light_instances[i].getAllVertices()[j].drawableDataID += baseObjectVertex[p];
                        vertexCount++;
                        vbBindlessTex[p].append(m_light_instances[i].getAllVertices()[j]);
                    }
                    drawArraysIndirectCommand.count = vertexCount;
            ...
            for (unsigned int i = 0; i < m_light_instances_indexed.size(); i++) {
               ...
               if ( m_light_instances_indexed[i].getAllVertices().getVertexCount() > 0) {
                    ...
                    for (unsigned int j = 0; j < m_light_instances_indexed[i].getAllVertices().getVertexCount(); j++) {
                        ...
                        m_light_instances_indexed[i].getAllVertices()[j].drawableDataID += baseObjectVertex[p];
        ...
        LightRenderComponent::~LightRenderComponent() {
            ...
            for (size_t i = 0; i < modelDataShaderStorageBuffers.size(); i++) {
                ...
                vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[1], lightDepthCommandBuffer.size(), lightDepthCommandBuffer.data());
                vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[2], depthCommandBuffer.size(), depthCommandBuffer.data());
                vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[3], alphaCommandBuffer.size(), alphaCommandBuffer.data());
                vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[4], specularCommandBuffer.size(), specularCommandBuffer.data());
                vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[5], bumpCommandBuffer.size(), bumpCommandBuffer.data());
                vkFreeCommandBuffers(vkDevice.getDevice(), secondaryBufferCommandPools[6], lightCommandBuffer.size(), lightCommandBuffer.data());
                for (unsigned int i = 0; i < secondaryBufferCommandPools.size(); i++) {
