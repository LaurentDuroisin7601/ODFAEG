...
namespace odfaeg
{
    namespace graphic {
        #ifdef VULKAN
        RenderTexture::RenderTexture(window::Device& vkDevice) : RenderTarget(vkDevice), vkDevice(vkDevice), isCubeMap(false) {
        ...
        bool RenderTexture::create(unsigned int width, unsigned int height) {
            ...
            m_textures.resize(NB_SWAPCHAIN_IMAGES, texture);
            for (unsigned int i = 0; i < m_textures.size(); i++)
                m_textures[i].create(width, height, true);
        ...
        bool RenderTexture::createCubeMap(unsigned int width, unsigned int height) {
            ...
            m_textures.resize(NB_SWAPCHAIN_IMAGES, texture);
            for (unsigned int i = 0; i < m_textures.size(); i++)
                m_textures[i].createCubeMap(width, height, true);
        ...
        void RenderTexture::createSyncObjects() {
            ...
            imageAvailableSemaphores.resize(getSwapchainImagesSize());
            ...
            for (size_t i = 0; i < getSwapchainImagesSize(); i++) {
                if (vkCreateSemaphore(vkDevice.getDevice(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS)
                    throw core::Erreur(0, "échec de la création des objets de synchronisation pour une image!", 1);
            }
        }
        ...
        VkFormat RenderTexture::getSwapchainImageFormat() {
            return m_textures[currentFrame].getFormat();
        }
        std::vector<VkImage> RenderTexture::getSwapchainImages() {
            std::vector<VkImage> images;
            for (unsigned int i = 0; i < m_textures.size(); i++)
                images.push_back(m_textures[i].getImage());
            return images;
        }
        ...
        const Texture& RenderTexture::getTexture(unsigned int currentFrame) const {
        ...
        const int RenderTexture::getSwapchainImagesSize() {
        ...
        void RenderTexture::submit(bool lastSubmit, std::vector<VkSemaphore> signalSemaphores,
            ...
            if (getCommandBuffers().size() > 0) {
                ...
                if (firstSubmit) {
                    ...
                    waitSemaphores.push_back(imageAvailableSemaphores[imageIndex]);
                    waitStages.push_back(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
                    waitValues.push_back(value[imageIndex]);
                ...
                if (lastSubmit) {
                    ...
                    signalSemaphores.push_back(imageAvailableSemaphores[imageIndex]);
                    signalValues.push_back(++value[imageIndex]);
