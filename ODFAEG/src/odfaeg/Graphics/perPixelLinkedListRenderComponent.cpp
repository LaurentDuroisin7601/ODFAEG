            ...
            if (useThread) {
                ...
                for (unsigned int i = 0; i < visibleEntities.size(); i++) {
                    if (visibleEntities[i] != nullptr && (visibleEntities[i]->getType() == "E_PARTICLES"
                        ...
                        visibleEntities[i]->computeParticles(&mtx2, &cv2, vbBindlessTex[Triangles], frameBuffer.getCurrentFrame(),visibleEntities[i]->getChildTransform(), (visibleEntities[i]->getDrawMode() == Entity::INSTANCED) ? true : false, computeSemaphores[i][frameBuffer.getCurrentFrame()], computeFences[i][frameBuffer.getCurrentFrame()]);
