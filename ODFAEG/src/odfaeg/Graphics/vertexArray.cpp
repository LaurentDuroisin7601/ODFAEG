...
namespace odfaeg {
    namespace graphic {
        ...
        ////////////////////////////////////////////////////////////
        VertexArray::VertexArray(PrimitiveType type, unsigned int vertexCount, Entity* entity, entt::entity entityId) :
        ...
        void VertexArray::setEntityId(entt::entity entity) {
            m_entityId = entity;
        }
        entt::entity VertexArray::getEntityId() {
            return m_entityId;
        }
        ...
        void VertexArray::computeNormals() {
           if (needToUpdateNormals) {
                unsigned int size = 0;
                if (m_primitiveType == Quads) {
                    size = m_vertices.size() / 4;
                } else if (m_primitiveType == Triangles) {
                    size = m_vertices.size() / 3;
                } else if (m_primitiveType == TriangleStrip || m_primitiveType == TriangleFan) {
                ...
                for (unsigned int i = 0; i < size; i++) {
                    if (m_primitiveType == Quads) {
                        for (unsigned int n = 0; n < 4; n++) {
                            math::Vec3f v1 (m_vertices[i*4+n].position);
                            math::Vec3f v2;
                            math::Vec3f v3;
                            if (n == 0) {
                                v2 = m_vertices[i*4+3].position;
                            } else {
                                v2 = m_vertices[i*4+n-1].position;
                            }
                            if (n == 3) {
                                v3 = m_vertices[i*4].position;
                            } else {
                                v3 = m_vertices[i*4+n+1].position;
                            }
                            math::Vec3f dir1 = v2 - v1;
                            math::Vec3f dir2 = v3 - v1;
                            math::Vec3f normal = dir1.cross(dir2).normalize();
                            m_normals[i*4+n] = normal;
                        }
                    } else if (m_primitiveType == Triangles) {
                        for (unsigned int n = 0; n < 3; n++) {
                            math::Vec3f v1 (m_vertices[i*3+n].position);
                            math::Vec3f v2;
                            math::Vec3f v3;
                            if (n == 0) {
                                v2 = m_vertices[i*3+2].position;
                            } else {
                                v2 = m_vertices[i*3+n-1].position;
                            }
                            if (n == 2) {
                                v3 = m_vertices[i*3].position;
                            } else {
                                v3 = m_vertices[i*3+n+1].position;
                            }
                            math::Vec3f dir1 = v2 - v1;
                            math::Vec3f dir2 = v3 - v1;
                            math::Vec3f normal = dir1.cross(dir2).normalize();
                            m_normals[i*3+n] = normal;
                        }
                    } else if (m_primitiveType == TriangleStrip) {
                        if (i == 0) {
                            for (unsigned int n = 0; n < 3; n++) {
                                math::Vec3f v1 (m_vertices[n].position);
                                math::Vec3f v2;
                                math::Vec3f v3;
                                if (n == 0) {
                                    v2 = m_vertices[2].position;
                                } else {
                                    v2 = m_vertices[n-1].position;
                                }
                                if (n == 2) {
                                    v3 = m_vertices[0].position;
                                } else {
                                    v3 = m_vertices[n+1].position;
                                }
                                math::Vec3f dir1 = v2 - v1;
                                math::Vec3f dir2 = v3 - v1;
                                math::Vec3f normal = dir1.cross(dir2).normalize();
                            ...
                            math::Vec3f v1 (m_vertices[i+2].position);
                            math::Vec3f v2 (m_vertices[i+1].position);
                            math::Vec3f v3 (m_vertices[i].position);
                            math::Vec3f dir1 = v2 - v1;
                            math::Vec3f dir2 = v3 - v1;
                            math::Vec3f normal = dir1.cross(dir2).normalize();
                            m_normals[i+3] = normal;
                        }
                        ...
                        if (i == 0) {
                            ...
                            math::Vec3f v1 (m_vertices[i+2].position);
                            math::Vec3f v2 (m_vertices[i+1].position);
                            math::Vec3f v3 (m_vertices[0].position);
                            math::Vec3f dir1 = v2 - v1;
                            math::Vec3f dir2 = v3 - v1;
                            math::Vec3f normal = dir1.cross(dir2).normalize();
                            m_normals[i+3] = normal;
        ...
        void VertexArray::updateNormals() {
            if (needToUpdateNormals) {
                computeNormals();
                needToUpdateNormals = false;
            }
