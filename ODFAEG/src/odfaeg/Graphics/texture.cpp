...
namespace odfaeg {
    namespace graphic {
        ...
        isCubeMap(copy.isCubeMap),
        isFBOTexture(copy.isFBOTexture),
        ct(copy.ct)
        {
            createCommandPool();
            ...
            {
                if (copy.isCubeMap) {
                    if (createCubeMap(copy.getSize().x(), copy.getSize().y(), copy.isFBOTexture))
                    {
                        updateCubeMap(copy);
                    ...
                    if (create(copy.getSize().x(), copy.getSize().y(), copy.isFBOTexture))
                    {
                        update(copy);
                        // Force an OpenGL flush, so that the texture will appear updated
                        // in all contexts immediately (solves problems in multi-threaded apps)
        ...
        bool Texture::loadCubeMapFromImage(const Image& image, const IntRect& area, uint32_t face) {
        ...
        math::Vector2u Texture::getSize() const {
            return math::Vector2u(m_size.x(), m_size.y());
        }
        ...
        void Texture::setNativeHandle(unsigned int handle, unsigned int width, unsigned int height) {
            m_texture = handle;
            m_actualSize[0] = width;
            m_actualSize[1] = height;
        }
        ...
        bool Texture::createCubeMap(unsigned int width, unsigned int height, std::vector<Image> images) {
            m_isCubeMap = true;
            // Check if texture parameters are valid before creating it
            if ((width == 0) || (height == 0))
            {
