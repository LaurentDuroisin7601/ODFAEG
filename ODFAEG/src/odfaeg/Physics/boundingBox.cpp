...
namespace odfaeg {
    namespace physic {
        ...
        BoundingBox::BoundingBox (float x, float y, float z, float width, float height, float depth) : BoundingVolume(), center (x + width * 0.5f, y + height * 0.5f, z + depth * 0.5f) {
            this->x = x;
            this->y = y;
            this->z = z;
            this->width = width;
        ...
        bool BoundingBox::intersects (BoundingBox &other, CollisionResultSet::Info& info) {
            ...
            float x, y, z;
            if (center.x - minX1 < maxX1 - center.x)
                x = -(center.x - minX1);
            else
                x = maxX1 - center.x;
            if (center.y - minY1 < maxY1 - center.y)
                y = - (center.y - minY1);
            else
                y = maxY1 - center.y;
            if (center.z - minZ1 < maxZ1 - center.z)
                z = -(center.z - minZ1);
            else
                z = maxZ1 - center.z;
            info.mtu = math::Vec3f(x, y, z);
        ...
        BoundingBox BoundingBox::inverseTransform(graphic::TransformMatrix& tm) {
            std::array<math::Vec3f, 8> points;
            //Devant
            points[0] = getPosition();
            points[1] = math::Vec3f (getPosition().x + getWidth(), getPosition().y, getPosition().z);
