...
namespace odfaeg
{
    namespace physic {
        ...
        , mParticleLifetime(core::seconds(1.f))
        , mParticlePosition(math::Vec3f(0.f, 0.f, 0.f))
        , mParticleVelocity(math::Vec3f(0.f, 0.f, 0.f))
        , mParticleRotation(0.f)
        , mParticleRotationSpeed(0.f)
        , mParticleScale(math::Vec3f(1.f, 1.f, 1.f))
        , mParticleColor(graphic::Color::White)
        , mParticleTextureIndex(0u)
        {
        }

        void UniversalEmitter::operator() (EmissionInterface& system, core::Time dt)
        {
        ...
        void UniversalEmitter::setParticleLifetime(math::Distribution<core::Time> particleLifetime)
        {
            mParticleLifetime.swap(particleLifetime);
        }

        void UniversalEmitter::setParticlePosition(math::Distribution<math::Vec3f> particlePosition)
        {
            mParticlePosition.swap(particlePosition);
        }

        void UniversalEmitter::setParticleVelocity(math::Distribution<math::Vec3f> particleVelocity)
        {
        ...
        void UniversalEmitter::setParticleScale(math::Distribution<math::Vec3f> particleScale)
        {
            mParticleScale.swap(particleScale);
        }

        void UniversalEmitter::setParticleColor(math::Distribution<graphic::Color> particleColor)
        {
        ...
        unsigned int UniversalEmitter::computeParticleCount(core::Time dt)
        {
