...
layout (binding = 0, set =  1) uniform buffer vertexBuffer {
    Vertex vertices[];
};
layout (binding = 1, set = 1) uniform buffer indexBuffer {
    uint indexes[];
};
layout (binding = 2, set = 1) uniform buffer geomBuffer {
    GeometryOffset geomOffsets[];
};
layout (binding = 3, set = 1) uniform buffer materialBuffer {
...
void main() {
    ...
    MaterialData material = materials[geomOffs.materialOffset].textureMatrix;
