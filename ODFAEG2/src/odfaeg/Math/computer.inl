namespace odfaeg {
   namespace math {
        ...
        std::array<std::array<float, 2>, 2> Computer::getExtends(const std::array<Vec2f, N>& verts) {
            float minX = 0;
            float maxX = 0;
            float minY = 0;
            float maxY = 0;
        ...
        */
        template <std::size_t N>
        std::array<std::array<float, 2>, 3> Computer::getExtends(const std::array<Vec3f, N>& verts) {
            float minX = 0;
            float maxX = 0;
            float minY = 0;
            float maxY = 0;
        ...
        */
        template <typename T>
        Vec3f Computer::getPosOnPathFromTime(Vec3f actualPos, std::vector<Vec3f> path, T time, float speed) {
        ...
        template <typename T>
        Vec2f Computer::getPosOnPathFromTime(Vec2f actualPos, std::vector<Vec2f> path, T time, float speed) {
        ...
        template <std::size_t N>
        Vec2f Computer::projectShapeOnAxis(Vec3f axis, std::array<Vec3f, N> vertices) {
            float min = 0, max = 0;
            if (vertices.size() > 0) {
                min = vertices[0].projOnAxis(axis);
                max = min;
