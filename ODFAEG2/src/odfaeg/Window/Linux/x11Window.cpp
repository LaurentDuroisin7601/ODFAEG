...
module odfaeg.window.x11Window;
...
namespace {
    ...
    odfaeg::core::String                            windowManagerName;

    odfaeg::core::String                            wmAbsPosGood[] = { "Enlightenment", "FVWM", "i3" };
    ...
    bool ewmhSupported()
    {
        ...
        if (actualType && numItems)
        {
            ...
            windowManagerName = odfaeg::core::String::fromUtf8(begin, end);
        }
        if (result == Success)
            XFree(data);
        odfaeg::window::Display::closeDisplay(display);
...
namespace odfaeg {
    namespace window {
        ...
        ////////////////////////////////////////////////////////////
        X11Window::X11Window(WindowHandle handle) :
        m_window         (0),
        m_screen         (0),
        m_inputMethod    (NULL),
        m_inputContext   (NULL),
        ...
        X11Window::X11Window(VideoMode mode, const core::String& title, std::uint32_t style) :
        m_window         (0),
        m_screen         (0),
        m_inputMethod    (NULL),
        m_inputContext   (NULL),
        ...
        void X11Window::create(WindowHandle handle) {
        ...
        void X11Window::create (VideoMode mode, const core::String& title, std::uint32_t style) {
            ...
            m_window = XCreateWindow(m_display, DefaultRootWindow(m_display),
                                          windowPosition.x(), windowPosition.y(), mode.width, mode.height, 0, depth, InputOutput,
                                          visual,
        ...
        void X11Window::initialize() {
            ...
            std::lock_guard<std::recursive_mutex> lock(allWindowsMutex);
            allWindows.push_back(this);
            m_opened = true;
            m_destroyed = false;
        }
        void X11Window::setPosition(const math::Vector2i& position) {
            XMoveWindow(m_display, m_window, position.x(), position.y());
            XFlush(m_display);
        }
        math::Vector2i X11Window::getPosition() const {
            ...
            if (isWMAbsolutePositionGood())
                return math::Vector2i(static_cast<int>(xAbsRelToRoot), static_cast<int>(yAbsRelToRoot));
            ...
            if (getEWMHFrameExtents(m_display, m_window, xFrameExtent, yFrameExtent))
            {
                ...
                return math::Vector2i(static_cast<int>(xAbsRelToRoot - xFrameExtent), static_cast<int>(yAbsRelToRoot - yFrameExtent));
            }
            ...
            return math::Vector2i(xRelToRoot, yRelToRoot);
        }
        math::Vector2u X11Window::getSize() const
        {
            XWindowAttributes attributes;
            XGetWindowAttributes(m_display, m_window, &attributes);
            return math::Vector2u(static_cast<unsigned int>(attributes.width), static_cast<unsigned int>(attributes.height));
        }
        void X11Window::setSize(const math::Vector2u& size)
        {
        ...
        void X11Window::setIcon(unsigned int width, unsigned int height, const std::uint8_t* pixels)
        {
        ...
        void X11Window::setMouseCursor(const X11Cursor& cursor) {
            m_lastCursor = cursor.getCursor();
        ...
        void X11Window::requestFocus()
        {
            ...
            {
                std::cerr << "Failed to check if window is viewable while requesting focus" << std::endl;
        ...
        void X11Window::grabFocus()
        {
            ...
            if (netActiveWindow)
            {
                ...
                if (!result)
                    std::cerr<< "Setting fullscreen failed, could not send \"_NET_ACTIVE_WINDOW\" event" << std::endl;
            }
            else
            {
        ...
        void X11Window::setVideoMode(const VideoMode& mode)
        {
            ...
            if (!res)
            {
                std::cerr << "Failed to get the current screen resources for fullscreen mode, switching to window mode" << std::endl;
                return;
            }
            ...
            if (!outputInfo || outputInfo->connection == RR_Disconnected)
            {
                ...
                std::cerr << "Failed to get output info for fullscreen mode, switching to window mode" << std::endl;
                return;
            }
            ...
            if (!crtcInfo)
            {
                XRRFreeScreenResources(res);
                XRRFreeOutputInfo(outputInfo);
                std::cerr << "Failed to get crtc info for fullscreen mode, switching to window mode" << std::endl;
                return;
            }
            ...
            if (!modeFound)
            {
                XRRFreeScreenResources(res);
                XRRFreeOutputInfo(outputInfo);
                std::cerr<< "Failed to find a matching RRMode for fullscreen mode, switching to window mode" << std::endl;
                return;
            }
        ...
        void X11Window::resetVideoMode()
        {
            if (fullscreenWindow == this)
            {
                ...
                if (checkXRandR(xRandRMajor, xRandRMinor))
                {
                    ...
                    if (!res)
                    {
                        std::cerr<< "Failed to get the current screen resources to reset the video mode" << std::endl;
                        return;
                    }
                    ...
                    {
                        XRRFreeScreenResources(res);
                        std::cerr << "Failed to get crtc info to reset the video mode" << std::endl;
                        return;
                    }
        ...
        void X11Window::switchToFullscreen()
        {
            ...
            if (ewmhSupported())
            {
                ...
                {
                    std::cerr << "Setting fullscreen failed. Could not get required atoms" << std::endl;
                    return;
                }
        ...
        void X11Window::setProtocols()
        {
            ...
            {
                std::cerr<< "Failed to request WM_DELETE_WINDOW atom." << std::endl;
            }
        ...
        void X11Window::setTitle(const core::String& title) {
              // Bare X11 has no Unicode window title support.
              // There is however an option to tell the window manager your Unicode title via hints.

              // Convert to UTF-8 encoding.
              std::basic_string<std::uint8_t> utf8Title;
              core::Utf32::toUtf8(title.begin(), title.end(), std::back_inserter(utf8Title));
        ...
        WindowHandle X11Window::getSystemHandle() const {
            return m_window;
        }
                ...
                if (!XFilterEvent(&xevent, None))
                {
                    ...
                    if (m_inputContext)
                    {
                        ...
                        if (length > 0)
                        {
                            ...
                            core::Utf8::decode(keyBuffer, keyBuffer + length, unicode, 0);
                            if (unicode != 0)
                            {                                ;
                                IEvent textEvent;
        ...
        bool X11Window::popEvent(IEvent& event, bool block) {
        ...
        bool X11Window::checkXRandR(int& xRandRMajor, int& xRandRMinor)
        {
            ...
            {
                std::cerr << "XRandR is too old" << std::endl;
                return false;
            }
        ...
        ////////////////////////////////////////////////////////////
        math::Vector2i X11Window::getPrimaryMonitorPosition()
        {
            math::Vector2i monitorPosition;
            ...
            if (!res)
            {
                std::cerr << "Failed to get the current screen resources for.primary monitor position" << std::endl;
            ...
            if (!outputInfo || outputInfo->connection == RR_Disconnected)
            {
                ...
                std::cerr << "Failed to get output info for.primary monitor position" << std::endl;
                return monitorPosition;
            }
            ...
            if (!crtcInfo)
            {
                XRRFreeScreenResources(res);
                XRRFreeOutputInfo(outputInfo);
                std::cerr << "Failed to get crtc info for.primary monitor position" << std::endl;
                return monitorPosition;
            }
        ...
        VkSurfaceKHR X11Window::createSurface(VkInstance instance) {
            ...
            surfaceInfo.dpy    = m_display;   // ton Display*
            surfaceInfo.window = m_window;   // ton Window
        ...
        void X11Window::getFramebufferSize(int& width, int& height) {
...
// Created by laurent on 22/05/2026.
//
