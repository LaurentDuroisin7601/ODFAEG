...
namespace odfaeg {
    namespace entity {
        ...
        */
        ...
        Tile::Tile (std::string textureId, math::Vec3f position, math::Vec3f size, FloatRect subRect, Color color, GameObject *parent)
        ...
        void Tile::setColor (Color color) {
            for (unsigned int j = 0; j < getSubMeshes()[0].getVertexArray().getVertexCount(); j++)
                getSubMeshes()[0].getVertexArray()[j].color = color;
        }
        Color Tile::getColor() {
            return getSubMeshes()[0].getVertexArray()[0].color;
        }
        FloatRect Tile::getTexCoords() {
            return texRect;
        }
        void Tile::setTexRect(FloatRect rect) {
            VertexArray& va = getSubMeshes()[0].getVertexArray();
            va[0].texCoords = math::Vec2f(static_cast<float>(rect.left), static_cast<float>(rect.top));
            va[1].texCoords = math::Vec2f(static_cast<float>(rect.left + rect.width), static_cast<float>(rect.top));
            va[2].texCoords = math::Vec2f(static_cast<float>(rect.left + rect.width), static_cast<float>(rect.top + rect.height));
            va[3].texCoords = math::Vec2f(static_cast<float>(rect.left), static_cast<float>(rect.top + rect.height));
        }
    }
}
