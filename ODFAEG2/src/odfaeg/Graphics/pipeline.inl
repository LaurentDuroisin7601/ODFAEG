namespace odfaeg {
	namespace graphic {	
		...
		void Pipeline::createGraphicPipeline(Shader& shader, std::deque<DescriptorSetLayout>& setLayouts, VkPipelineRenderingCreateInfo renderingCreateInfo, VkPipelineDepthStencilStateCreateInfo depthStencil, BlendMode blendMode,
			...
			meshShaderModule = shader.getMeshShaderModule();
			fragmentShaderModule = shader.getFragmentShaderModule();
			taskShaderModule = shader.getTaskShaderModule();
			std::vector<VkPipelineShaderStageCreateInfo> shaderStages;
		...
		void Pipeline::createRTPipeline(Shader& shader, std::deque<DescriptorSetLayout>& setLayouts, std::vector<VkPushConstantRange> pushConstants) {
			...
			if (shader.getRayAnyhitShaderModule() != VK_NULL_HANDLE) {
				...
				chitshaderStageInfo.module = shader.getRayhitShaderModule();
				...
				anyhitshaderStageInfo.module = shader.getRayAnyhitShaderModule();
