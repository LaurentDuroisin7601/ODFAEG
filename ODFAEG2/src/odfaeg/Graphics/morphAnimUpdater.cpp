    ...
    namespace graphic {
        ...
        void MorphAnimUpdater::onUpdate() {
            ...
            if (isRunning()) {
                ...
                /*std::cout << "morph anim thread " << std::this_thread::get_id()
                << " calling vkQueueSubmit" << std::endl;*/
                Device::QueueFamilyIndices indices = GPUContext::instance().getDevice().findQueueFamilies(GPUContext::instance().getDevice().getPhysicalDevice());
                if (vkQueueSubmit(GPUContext::instance().getDevice().getQueue(indices.graphicsFamily.value(), 2), 1, &submitInfo, GPUContext::instance().getSharedFence(1)[0].getHandle()) != VK_SUCCESS) {
