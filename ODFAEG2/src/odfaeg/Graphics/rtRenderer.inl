            ...
            for (unsigned int i = 0; i < gameObjects.size(); i++) {
                for (unsigned int j = 0; j < gameObjects[i]->getGameObject()->getSubMeshesCount(); j++) {
                    ...
                    if (!gameObjects[i]->getMaterials()[0]->getInstanceGroupId == -1) {
                        ...
                        gameObjects[i]->instanceId = instanceCount;                        
                        instanceCount++;                         
                    } 
                }
                ...
                for (unsigned int j = 0; j < gameObjects[i]->getGameObject()->getSubMeshesCount(); j++) {
                    ...
                    if (!gameObjects[i]->getMaterials()[0]->getInstanceGroupId != -1) {
                        if (gameObjects[i]->getMaterials()[0]->materialSet == 0) {
                            ...
                            gameObjects[i]->blasId = blasCount + gameObjects[i]->getMaterials()[0]->getInstanceGroupId();
                            VkAccelerationStructureGeometryKHR accelerationStructureGeometry{};
                            accelerationStructureGeometry.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_KHR;
                            accelerationStructureGeometry.flags = VK_GEOMETRY_OPAQUE_BIT_KHR;
                            accelerationStructureGeometry.geometryType = VK_GEOMETRY_TYPE_TRIANGLES_KHR;
                            ...
                            geometryOffset.vertexOffset = sm.vertexOffset;
                            geometryOffset.indexOffset = sm.indexOffset + lods[0].indexOffset;   
                            unsigned instanceID = instanceCount + gameObjects[i]->getMaterials()[0]->getInstanceGroupId();
                            if (instanceID >= geometryOffsets)
                                geometryOffsets.resize(instanceID);          
                            geometryOffsets[instanceID] = geometryOffset;
                        } 
                        gameObjects[i]->instanceId = instanceCount + gameObjects[i]->getMaterials()[0]->getInstanceGroupId();                        
                    } 
                }
            }            
        }
        void RTRenderer::updateTLAS() {
            ...
            for (unsigned int i = 0; i < gameObjects.size(); i++) {
                for (unsigned int j = 0; j < gameObjects[i]->getGameObject()->getSubMeshesCount(); j++) {
                    ...
                    instance.instanceCustomIndex = gameObjects[i]->instanceId;
                    instance.mask = 0xFF;
